...
def get_title(question_id):
    ...
    data = {
        'include': """data[*].is_normal,admin_closed_comment,reward_info,is_collapsed,annotation_action,
                    ...
                    relevant_info,question,excerpt,relationship.is_authorized,is_author,voting,is_thanked,is_nothelp,is_labeled,
                    is_recognized,paid_info,paid_info_content;data[*].mark_infos[*].url;data[*].author.follower_count,
        badge[*].topics""",
        'limit': 10,
        'offset': 0,
    }
    ...
    resp = response.json()
    title = resp.get('data')[0]['question']['title']
    return title
...
def zhihu_spider(question_id):
    ...
    while True:
        ...
        data = {
            'include': """data[*].is_normal,admin_closed_comment,reward_info,is_collapsed,annotation_action,
                        annotation_detail,collapse_reason,is_sticky,collapsed_by,suggest_edit,comment_count,can_comment,content,
                        editable_content,voteup_count,reshipment_settings,comment_permission,created_time,updated_time,
                        review_info,relevant_info,question,excerpt,relationship.is_authorized,is_author,voting,is_thanked,
                        is_nothelp,is_labeled,is_recognized,paid_info,paid_info_content;data[*].mark_infos[*].url;data[
        ...
        for answer in answers:
            ...
            for i in pic_urls:
                pic_num += 1
                with open(question_id + 'url.txt', 'a', encoding='utf-8') as f:
...
def zh_download(question_id):
    ...
    total = 1
    try:
        with open(question_id + 'url.txt', 'r', encoding='utf-8') as f:
            for i in f:
                img = i.replace('\n', '').split('\t')
                img_info.append(img)
                total += 1
    ...
    for i in img_info:
        ...
        img_author_name = i[1]
        # 获取当前文件所在目录
        path = os.path.dirname(os.getcwd()) + os.sep + title[:-1]
        # 创建文件夹 用来存抓取的图片
...
if __name__ == '__main__':
    ids = ['319371540', '368001626']
    # zh_download(question_id='368001626')
    pool = multiprocessing.Pool()
    pool.map(zh_download, ids)
    pool.close()
    pool.join()
